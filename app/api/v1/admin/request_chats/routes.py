...
from app.api.v1.admin.request_chats.schemas import AdminNoteRequest, AdminRequestChatResponse, ConvertToTemplateRequest
...
@router.get("", response_model=SuccessResponse[list[AdminRequestChatResponse]], summary="List request chats for admin review", description="Returns sanitized request chat outputs for product, support, and abuse review. Requires Bearer token with SUPER_ADMIN role.")
...
@router.get("/{chat_id}", response_model=SuccessResponse[AdminRequestChatResponse | None], summary="Get request chat for admin review", description="Returns a single sanitized request chat. Requires Bearer token with SUPER_ADMIN role.")
